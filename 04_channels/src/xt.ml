...
let block_on_log (log : entry list) : unit =
  if log = [] then ()
    ...
    let awaiter () =
      if not (Atomic.exchange woken true) then begin
        Mutex.lock mutex;
        Condition.signal cond;
        Mutex.unlock mutex
      end
    in
    (* Register awaiter on every location in the log. *)
    ...
    (* Block until woken. *)
    if not (Atomic.get woken) then begin
      Mutex.lock mutex;
      while not (Atomic.get woken) do
